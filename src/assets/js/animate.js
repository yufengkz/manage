...
*/
  var css = window.getComputedStyle
    ...
    : function(a, b, c) {
        if (c == undefined) {
          if (b == "opacity") {
            return a.style.filter.indexOf("opacity=") >= 0
              ? parseFloat(a.style.filter.match(/opacity=([^)]*)/)[1]) / 100
              : "1";
          } else {
...
*/
  var getJsonMap = function(jsonMap) {
    ...
    for (a in jsonMap) {
      ...
      json.style = a;
      json.val =
        typeof jsonMap[a] == "number" ? jsonMap[a] : parseFloat(test[1]);
      json.px = test[3];
      arr[arr.length] = json;
    }
...
*/
  ...
  Animate.prototype = {
    ...
    * 主要实现方法
    */
    go: function(elem, style, val, callBack, time, px) {
      px = px || "";
      /*
    * 可能无法得到元素的真实值，
    ...
    */
    play: function() {
      ...
      for (; i < len; i++) {
        config.jsonMap[i].interval = self.go(
          elem,
          config.jsonMap[i].style,
          config.jsonMap[i].val,
          callBack,
          time,
          config.jsonMap[i].px,
          systemCallBack
    ...
    */
    stop: function(k) {
      ...
      for (; i < len; i++) {
        ...
        if (k == true) {
          config.ele.style[config.jsonMap[i].style] =
            config.jsonMap[i].val + config.jsonMap[i].px;
        }
      }
