    ...
    <div class="tabs">
      <div class="breadcrumb">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item :to="{ path: '' }">视频列表</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!-- 检索 -->
      <div class="media-manage-table material">
        <div class="search">
          ...
          <div>文章ID：</div>
          <el-input v-model="searchID" @keyup.enter.native='search' placeholder="请输入内容(精准搜索)" size="mini"></el-input>
          <el-button type="primary" size="mini" @click="search">检索</el-button>
          <el-button class='addArticle' type='primary' size='mini' @click="$router.push('/videoadd')">添加视频</el-button>
        </div>
        <!-- 列表 -->
        <el-table ref="multipleTable" v-loading='tableLoading1' :data="tableData1" key="table1" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="viewCount" label="点击">
          </el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button type="text" size="small" @click="ban(scope.row)">{{scope.row.status===1?'正常':'禁用'}}</el-button>
              <el-button type="text" size="small" @click="$router.push('/videoedit?id='+scope.row.id)">编辑</el-button>
