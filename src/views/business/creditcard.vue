  ...
  <div>
    <div class="tabs">
      ...
      <div class="media-manage-table material">
        <div class="search">
          <div>关键字：</div>
          <el-input v-model="keyword" @keyup.enter.native='search' placeholder="请输入关键字/项目标题" size="mini"></el-input>
          <!-- <div>ID：</div>
          <el-input v-model="searchID" placeholder="请输入ID" size="mini"></el-input> -->
          <el-button type="primary" size="mini" @click="search">搜索</el-button>
          <el-button class='addArticle' type='primary' size='mini' @click="$router.push('/creditcardadd')">添加项目</el-button>
        </div>
        <!-- 列表 -->
        <el-table class="credit" ref="multipleTable" v-loading='tableLoading1' :data="tableData1" key="table1" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="isHotcardDes" label="项目种类" width="80">
          </el-table-column>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button type="text" size="small" @click="$router.push('/creditcardedit?id='+scope.row.id)">管理链接</el-button>
              <el-button type="text" size="small" @click="ban(scope.row)">{{scope.row.status===1?'正常':'禁用'}}</el-button>
              <el-button type="text" size="small" @click="$router.push('/creditcardbankedit?id='+scope.row.id)">编辑</el-button>
