    ...
    <div class="tabs">
      <div class="breadcrumb">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item :to="{ path: '' }">管理人</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!-- 检索 -->
      <div class="media-manage-table material">
        <div class="search">
          <el-button class='addArticle' type='primary' size='mini' @click="$router.push('/managesingleadd')">+ 添加人</el-button>
        </div>
        <!-- 列表 -->
        <el-table ref="multipleTable" v-loading='tableLoading1' :data="tableData1" key="table1">
          ...
          <el-table-column label="最后登录时间">
            <temeplate slot-scope="scope">
              {{scope.row.lastLoginTime|timeClip}}
            </temeplate>
          </el-table-column>
          <el-table-column prop="lastLoginIp" label="最后登录地址"></el-table-column>
          <el-table-column prop="statusStr" label="状态"></el-table-column>
          <el-table-column prop="roleName" label="组别"></el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button type="text" size="small" @click="$router.push('/managesingleedit?id='+scope.row.id)">编辑</el-button>
  ...
  data() {
    return {
      ...
      urls: {
        ...
        users: "users/",
        // 删除
        modifyIsDeleted: "users/modifyIsDeleted"
      }
    };
  },
  mounted() {
