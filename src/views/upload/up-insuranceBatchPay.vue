  ...
  <div class="ad" v-loading='tableLoading1'>
    <div class="breadcrumb">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item :to="{ path: '' }">保险批量发钱</el-breadcrumb-item>
      </el-breadcrumb>
    </div>

    <div class="upFile">
      <label>选择保险产品：</label>
      <el-select v-loading='optLoading' @change="getList" v-model="insuranceId" placeholder="请选择">
        <el-option v-for="item in options" :key="item.id" :label="item.name" :value="item.id">
        </el-option>
      </el-select>
      <el-upload class="upload-demo" :action='urls.upExcel' :on-success='upOK' :on-remove="handleRemove" :file-list="fileList" :data="{insuranceId:insuranceId}" :headers='headers'>
    ...
    <div class="views">
      ...
      <el-table class="credit" ref="multipleTable" :data="tableData1" key="table1" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="insurantName" label="姓名"></el-table-column>
        <el-table-column prop="insurantMobile" label="手机号"></el-table-column>
        <el-table-column prop="insurantIdCard" label="身份证号"></el-table-column>
        <el-table-column prop="orderId" label="订单ID">
          <template slot-scope="scope">
            {{scope.row.status===1?scope.row.orderId:scope.row.statusDes}}
          </template>
        </el-table-column>
        <el-table-column prop="commissionY" label="佣金金额"></el-table-column>
        <el-table-column prop="dividendY" label="收入/进价（元）"></el-table-column>
        <el-table-column prop="statusDes" label="匹配状态"></el-table-column>
...
export default {
  data() {
    return {
      ...
      urls: {
        // 列表
        getFeedbackList: "insurances/getFeedbackList",
        // 上传数据
        upExcel: `${process.env.API_ROOT}insurances/upExcel`,
        // 支付
        excelPayMoney: "insurances/excelPayMoney",
        // 保险产品列表
        queryInsuranceList: "insurances/queryInsuranceList",
        // 匹配
        excelAction: "insurances/excelAction"
      },
      headers:{
  ...
  methods: {
    ...
    getOptList() {
      ...
      let body = {
        url: this.urls.queryInsuranceList
      };
