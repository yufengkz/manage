	...
	<div class="ad" v-loading='tableLoading1'>
		<div class="breadcrumb">
			<el-breadcrumb separator-class="el-icon-arrow-right">
				...
				<el-breadcrumb-item :to="{ path: '' }">贷款</el-breadcrumb-item>
			</el-breadcrumb>
		</div>

		<div class="upFile">
			<label>选择贷款产品：</label>
			<el-select v-loading='optLoading' @change="getList" v-model="loanId" placeholder="请选择">
				<el-option v-for="item in options" :key="item.id" :label="item.name" :value="item.id">
				</el-option>
			</el-select>
			<el-upload class="upload-demo" :action='urls.upExcel' :on-success='upOK' :on-remove="handleRemove"
			           :file-list="fileList" :data="{loanId:loanId}" :headers='headers'>
		...
		<div class="date">
			<div>上传时间选择：</div>
			<el-date-picker v-model="applyTime" size="mini" type="datetimerange" range-separator=" ~ " placeholder="请选择"
			                :editable="false" @change="getList" start-placeholder="开始日期" end-placeholder="结束日期"
			                :clearable="false">
			</el-date-picker>
		</div>
		<div class="filter">
			...
			<el-radio-group class="group1" v-model="paymentStatus" size="small" @change='getList'>
				...
				<el-radio-button :label="3">授信支付</el-radio-button>
			</el-radio-group>
			<br>
			<el-radio-group class="group" v-model="status" size="small" @change='getList'>
				...
				<el-radio-button :label="23">匹配的记录已处理</el-radio-button>
				<el-radio-button :label="24">授信佣金重复</el-radio-button>
			</el-radio-group>
		</div>
		<div class="views">
			<!-- 列表 -->
			<el-table class="credit" ref="multipleTable" :data="tableData1" key="table1"
				...
				<el-table-column prop="creditCommission" label="授信佣金"></el-table-column>
				...
				<el-table-column label="操作">
					<template slot-scope="scope">
						<el-button v-if="scope.row.paymentStatus===0 && scope.row.status===1" type="text" size="small"
						           @click="pay(scope.row.id)">支付
						</el-button>
						<div v-else>-</div>
					</template>
				</el-table-column>
			...
			<div class="tab-page">
				<el-pagination @size-change="handleSizeChange1" @current-change="handleCurrentChange1"
				               ...
				               :page-size='pageSize' layout="total, sizes, ->, prev, pager, next" :total='pageTotal1'>
				</el-pagination>
			</div>
		</div>
	</div>
	...
	export default {
		data() {
			return {
				...
				urls: {
					// 列表
					getFeedbackList: "loans/getFeedbackList",
					// 上传数据
					upExcel: `${process.env.API_ROOT}loans/upExcel`,
					// 支付
					excelPayMoney: "loans/excelPayMoney",
					// 贷款产品列表
					queryLoanList: "loans/queryLoanList",
					// 匹配
					excelAction: "loans/excelAction"
				},
				headers: {
