	...
	<div>
		<div class="tabs">
			...
			<div class="media-manage-table material">
				<div class="search">
					<div>姓名：</div>
					<el-input v-model="keyword" @keyup.enter.native='search' placeholder="请输入姓名" size="mini"></el-input>
					<div>昵称：</div>
					<el-input v-model="wechatName" @keyup.enter.native='search' placeholder="请输入昵称" size="mini"></el-input>
					<div>身份证：</div>
					<el-input v-model="searchID" :maxlength='18' @keyup.enter.native='search' placeholder="请输入身份证"
					          size="mini"></el-input>
					<div>工号：</div>
					<el-input v-model="workNumberS" @keyup.enter.native='search' placeholder="请输入工号"
					          size="mini"></el-input>
					<div>电话：</div>
					<el-input v-model="mobile" @keyup.enter.native='search' placeholder="请输入电话" size="mini"></el-input>
				...
				<el-table ref="multipleTable" v-loading='tableLoading1' :data="tableData1" key="table1"
					...
					<el-table-column prop="wechatAvatar" label="头像" width="80">
						<template slot-scope='scope'>
							<a :href="scope.row.wechatAvatar" target="_blank">
								<img :src="scope.row.wechatAvatar" width="60" alt="">
							</a>
						</template>
					</el-table-column>
					...
					<el-table-column prop="wechatName" label="昵称">
					</el-table-column>
					<el-table-column prop="identityDes" label="职务" width="80">
						<!-- <template slot-scope='scope'>
						  {{scope.row.identity + ' - ' + scope.row.identityDes}}
						</template> -->
					</el-table-column>
					<el-table-column prop="parentWorkNumber" label="上级工号">
					</el-table-column>
					<el-table-column prop="statusDes" label="状态" width="80">
						<!-- <template slot-scope='scope'>
					...
					<el-table-column label="注册来源">
						<template slot-scope="scope">
							{{scope.row.dataFrom|dataFrom}}
						</template>
					</el-table-column>
					<el-table-column label="操作">
						<template slot-scope="scope">
							<el-button type="text" size="small" @click="$router.push('/usersedit?id='+scope.row.id)">
								编辑
							</el-button>
							<el-button v-if="scope.row.identity===4" type="text" size="small"
							           @click="$router.push('/operatecenter?id='+scope.row.id+'&name='+scope.row.realName+'&idCard='+scope.row.idNumber+'&operationCenter='+scope.row.operationCenter)">
								运营中心
							</el-button>
						</template>
					</el-table-column>
